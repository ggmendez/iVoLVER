var DateGenerator = fabric.util.createClass(fabric.Rect, {
    isDateGenerator: true,
    ...
    toXML: function () {
        ...
        var theDateGenerator = this;
        var generatorNode = createXMLElement("numberGenerator");

        appendElementWithValue(generatorNode, "left", theDateGenerator.left);
        appendElementWithValue(generatorNode, "top", theDateGenerator.top);
        appendElementWithValue(generatorNode, "width", theDateGenerator.width);
        appendElementWithValue(generatorNode, "height", theDateGenerator.height);

        var outputPoint = theDateGenerator.outputPoint;
        appendElementWithValue(generatorNode, "xCoordinate", outputPoint.left);

        var minXNode = theDateGenerator.minX.value.toXML();
        var maxXNode = theDateGenerator.maxX.value.toXML();

        addAttributeWithValue(minXNode, "xmlID", theDateGenerator.minX.xmlID);
        addAttributeWithValue(maxXNode, "xmlID", theDateGenerator.maxX.xmlID);
    ...
    initialize: function (options) {
        ...
        this.addDateLimits(options.values, options.xmlIDs);
    ...
    addDateLimit: function (limitName, numericValue, xmlID) {
        var theGenerator = this;
    ...
    computeOutput: function (xCoordinate, shouldAnimate) {
        ...
        var theDateGenerator = this;
        var theOutputPoint = theDateGenerator.outputPoint;

        if (!xCoordinate) {
            xCoordinate = theOutputPoint.left;
        }

        var oldMin = theDateGenerator.minX.getCenterPoint().x;
        var oldMax = theDateGenerator.maxX.getCenterPoint().x;
        var newMin = theDateGenerator.minX.value.moment.valueOf();
        var newMax = theDateGenerator.maxX.value.moment.valueOf();
        var output = moment(changeRange(xCoordinate, oldMin, oldMax, newMin, newMax));

        var refreshCanvas = false;

        theDateGenerator.outputPoint.setValue(createDateAndTimeValue(output), refreshCanvas, shouldAnimate);
    },
    addDateLimits: function (values, xmlIDs) {
        ...
        if (values !== null && typeof values !== 'undefined') {
            minXValue = values.minX || new DateAndTimeData({theMoment: 0});
            maxXValue = values.maxX || new DateAndTimeData({theMoment: 100});
        } else {

            var minMoment = moment();
            minXValue = createDateAndTimeValue(minMoment);
            var maxMoment = moment().add(1, 'hours');
            maxXValue = createDateAndTimeValue(maxMoment);
        ...
        theGenerator.addDateLimit('minX', minXValue, xmlIDs ? xmlIDs['minX'] : null);
        theGenerator.addDateLimit('maxX', maxXValue, xmlIDs ? xmlIDs['maxX'] : null);
    },
    addOutputPoint: function (values, xmlIDs) {
        ...
        if (values !== null && typeof values !== 'undefined') {
            outputValue = values.output || new DateAndTimeData({theMoment: moment()});
        } else {
            outputValue = new DateAndTimeData({theMoment: moment().add(0.5, 'h')});
        }
...
function addDateGenerator(options) {
    ...
    var theGenerator = new DateGenerator(options);
...
function createDateGeneratorFromXMLNode(functionXmlNode) {
    ...
    return addDateGenerator(options);
